...
These tools process CSV input files. Each row must include a statement/response id and text via either `comment-id` and `comment_text`, or alternative aliases `participant_id` and `survey_text`. Here `participant_id` is the statement/response id (same role as `comment-id`), not the person; an optional `author-id` may identify the person. When both name sets are present, the JS column names take precedence. For deliberations without group information, vote counts should be set in columns titled `agrees`, `disagrees` and `passes`.  If you do not have vote information, these can be set to 0. For deliberations with group breakdowns, you can set the columns `{group_name}-agree-count`, `{group_name}-disagree-count`, `{group_name}-pass-count`.
