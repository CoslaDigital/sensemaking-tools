...
def get_bridging_scores(
  ...
  if scorer_type == "GEMINI":
    ...
    if not api_key:
      raise ValueError("api_key is required when scorer_type is PERSPECTIVE.")
...
if __name__ == "__main__":
  ...
  if args.scorer_type == "PERSPECTIVE":
    perspective_key = sensemaker_model_cli.resolve_cli_api_key_from_args(args)
    if not perspective_key:
      raise ValueError(
          "--api_key is required for PERSPECTIVE scorer_type."
      )
